...
func (m *defaultGoodsModel) Insert(ctx context.Context, data *Goods) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.GoodId, data.DeletedAt, data.UserId, data.GoodName, data.GoodPicture, data.GoodPrice, data.GoodDescribe, data.GoodType, data.CurrencyType, data.IsBargain, data.PurchaseWay, data.PurchaseTime, data.OptionalStatus, data.GoodStartTime)
	return ret, err
}
...
func (m *defaultGoodsModel) Update(ctx context.Context, newData *Goods) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.DeletedAt, newData.UserId, newData.GoodName, newData.GoodPicture, newData.GoodPrice, newData.GoodDescribe, newData.GoodType, newData.CurrencyType, newData.IsBargain, newData.PurchaseWay, newData.PurchaseTime, newData.OptionalStatus, newData.GoodStartTime, newData.GoodId)
	return err
}
