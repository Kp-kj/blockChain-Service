...
var (
	...
	yesterdayBillRowsWithPlaceHolder = strings.Join(stringx.Remove(yesterdayBillFieldNames, "`user_id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), "=?,") + "=?"
)

type (
	yesterdayBillModel interface {
		Insert(ctx context.Context, data *YesterdayBill) (sql.Result, error)
		FindOne(ctx context.Context, userId int64) (*YesterdayBill, error)
...
func (m *defaultYesterdayBillModel) Delete(ctx context.Context, userId int64) error {
	query := fmt.Sprintf("delete from %s where `user_id` = ?", m.table)
	_, err := m.conn.ExecCtx(ctx, query, userId)
	return err
}

func (m *defaultYesterdayBillModel) FindOne(ctx context.Context, userId int64) (*YesterdayBill, error) {
	query := fmt.Sprintf("select %s from %s where `user_id` = ? limit 1", yesterdayBillRows, m.table)
...
func (m *defaultYesterdayBillModel) Insert(ctx context.Context, data *YesterdayBill) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.UserId, data.YesterdayRewardQuantity, data.YesterdayExpenseQuantity)
	return ret, err
}

func (m *defaultYesterdayBillModel) Update(ctx context.Context, data *YesterdayBill) error {
	query := fmt.Sprintf("update %s set %s where `user_id` = ?", m.table, yesterdayBillRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, data.YesterdayRewardQuantity, data.YesterdayExpenseQuantity, data.UserId)
	return err
}
